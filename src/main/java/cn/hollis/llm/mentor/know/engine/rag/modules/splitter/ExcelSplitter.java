...
public class ExcelSplitter {
    ...
    /**
     * 编码检测：优先BOM检测，再尝试UTF-8，最后回退到GBK（中文Windows常用编码）
     */
    private Charset detectCharset(byte[] data) {
        ...
        // 无BOM时，先尝试UTF-8，如果包含乱码特征则回退到GBK
        String sample = new String(data, 0, Math.min(1024, data.length), StandardCharsets.UTF_8);
        if (sample.contains("�")) {
